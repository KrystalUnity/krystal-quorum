# Consensus Matching V3 Implementation Plan
...
The two rollback issues can match by shared gap term `recovery`. The two security issues do not match by absence intent because they share no gap term.
...
- Three-reviewer connected-component test passes where C joins by matching B, not representative A.
...
def test_connected_component_joins_by_non_representative_match():
    clusters = cluster_issues(
        [
            ...
            ("c", issue("B3", "Deployment failure recovery strategy is absent.")),
        ]
    )
...
- [ ] **Step 1: Copy V3 plan to Gex**

Run:

```bash
scp docs/superpowers/plans/2026-06-23-consensus-matching-v3.md \
  gex44:/root/krystal-quorum/data/spec_reviews/2026-06-23-consensus-matching-v3.md
```
...
uv run krystal-quorum review \
  /root/krystal-quorum/data/spec_reviews/2026-06-23-consensus-matching-v3.md \
...
Expected: no `BLOCK` verdict. If the V3 plan blocks, update the design before implementing.
