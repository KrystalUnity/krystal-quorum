# Consensus Matching V5 Implementation Plan
...
## V5 Fixes From V4 Quorum Block

The V4 review found that the default test section `"Plan"` was treated as a rollback gap term through `"plan": "recovery"`. V5 resolves this with two exact rules:

1. Section text can score only broad topic aliases, never topic gap aliases.
2. `"plan"` is not a rollback gap alias. A rollback plan finding is still classified as rollback because the claim contains the broad topic alias `"rollback"`.
...
- [ ] **Step 1: Copy V5 plan to Gex**

Run:

```bash
scp docs/superpowers/plans/2026-06-23-consensus-matching-v5.md \
  gex44:/root/krystal-quorum/data/spec_reviews/2026-06-23-consensus-matching-v5.md
```
...
uv run krystal-quorum review \
  /root/krystal-quorum/data/spec_reviews/2026-06-23-consensus-matching-v5.md \
...
Expected: no `BLOCK` verdict. If the V5 plan blocks, update the design before implementing.
